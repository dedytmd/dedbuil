...
def mainlist(item):
    ...
    if txt_status:
        ...
        if con_incidencias:
            itemlist.append(item.clone( channel='submnuteam', action='resumen_incidencias', title=' - [B]Con Incidencias[/B]', text_color='tan', thumbnail=config.get_thumb('stack') ))

        if no_accesibles:
            itemlist.append(item.clone( channel='submnuteam', action='resumen_no_accesibles', title=' - [B]No Accesibles[/B]', text_color='indianred', thumbnail=config.get_thumb('stack') ))
